...
do 
   echo $PRODUCTNAME $c
   /usr/local/ecas/filemgr/bin/filemgr-client --url http://localhost:9000 --operation --getFirstPage --productTypeName $PRODUCTNAME | awk -F, '{ print $1 }' | awk -F= '{ print $2 }' > /tmp/productListing
