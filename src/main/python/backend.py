...
This is the backend storage for the EDRN Catalog and Archive System.  It uses the
...
from gov.nasa.jpl.edrn.ecas.backend import WebServer, Dispatcher, Result
...
class User:
	'''A user of the E-CAS.  Users don't have permissions directly; instead they receive
...
class Group:
	'''An E-CAS group.  The group contains a sequence of permissions, which are strings
...
def _setJavaProperties(config):
	...
	System.setProperty('gov.nasa.jpl.oodt.cas.filemgr.catalog.lucene.indexPath', config.get('index', 'path'))
